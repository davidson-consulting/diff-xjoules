...
pub fn read_json<T: DeserializeOwned>(path_to_json_file: String) -> T {
    let json_file_content = fs::read_to_string(path_to_json_file + JSON_EXTENSION).unwrap();
...
mod tests {
    ...
    #[test]
    fn test_read_json() {
        let coverage: Coverage = read_json::<Coverage>(String::from("test_resources/coverage"));
