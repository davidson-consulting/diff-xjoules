...
mod tests {
    ...
    fn test_read_yaml() {
        ...
        assert_eq!("java -jar diff-jjoules/target/diff-jjoules-1.0.0-SNAPSHOT-jar-with-dependencies.jar --path-to-project {{ path_project }} --second-path-to-project {{ second_path_project }} --task TEST_COVERAGE --output-path {{ output_path }}", configuration.coverage_cmd);
    }
}
