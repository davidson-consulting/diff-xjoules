...
mod tests {
    ...
    fn test_read_yaml() {
        ...
        assert_eq!("java -jar diff-jjoules/target/diff-jjoules-0.1.0-SNAPSHOT-jar-with-dependencies.jar -p {{ path_project }} --task TEST_COVERAGE --output-path {{ output_path }}", configuration.coverage_cmd);
    }
}
