 ...
 */
function usage() {
	return "Invalid script usage! node changelog.js <owner/project> <tag> <previous-version>";
}
...
function main() {
    ...
    parseCommitLog(process.argv[2], process.argv[3], process.argv[4], function (data) {});
}
 ...
 */
function parseCommitLog(repository, tag, version, callback) {
    ...
    log
        .parse({
            "_":  (tag == " " ? version : tag + "-" + version) + "..main"
        })
        .pipe(through2.obj(function (commit, enc, callback) {
            ...
            if (!found) {
                var url = GITHUB_WEBSITE + repository + "branch_commits/" + commit.commit.long;
...
function getCommitCategory(commit, categorizedCommits) {
    ...
    if (commit.subject.indexOf("[maven-release-plugin]") > -1) {
            return null;
    }
    return "unknown";
}
